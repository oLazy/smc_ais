...
DO iping = 2,NPING
  ...
  IF (rank == src) THEN
     ...
     OPEN(NEWUNIT(upartcl),FILE='particleBefore_particles.txt',FORM='formatted',STATUS='REPLACE', &
          ACTION='WRITE',RECL=4096)
     DO ipart = 1,NPARTAIS
        WRITE(upartcl,202) particles(ipart)%logL, particles(ipart)%logPr, particles(ipart)%lognorm, &
             REAL(particles(ipart)%k,RP),particles(ipart)%par,particles(ipart)%sdpar
     ENDDO
     CLOSE(upartcl)

     !! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!
     !!                   RESAMPLE TESTS!                                         !!
     CALL RESAMPLE_PER_K(particles(1:NPARTAIS),particles_new(1:NPARTAIS),kcount,kidx)!!
     !!                                                                           !!
     !! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!!

     OPEN(NEWUNIT(upartcl),FILE='particleAfter_particles.txt',FORM='formatted',STATUS='REPLACE', &
          ACTION='WRITE',RECL=4096)
     DO ipart = 1,NPARTAIS
...
!!=======================================================================
FUNCTION LOGFACTORIAL(n)  RESULT(fact)
!-----Factorial------------------------------------------------------
!!=======================================================================
